        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex m-2 p-2">
                <a href="{{ route('admin.rooms.index') }}"
                    class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">room Index</a>
            </div>
                ...
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.rooms.update', $room->id) }}"
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div>
                                <img class="w-32 h-32" src="{{ Storage::url($room->image) }}">
                            </div>
                            <div class="mt-1">
                            ...
                            <div class="mt-1">
                                <select id="status" name="status" class="form-multiselect block w-full mt-1">
                                    @foreach (App\Enums\RoomStatus::cases() as $status)
                                        <option value="{{ $status->value }}" @selected($room->status->value == $status->value)>
                                            {{ $status->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mt-1">
                                <select id="categories" name="categories[]" class="form-multiselect block w-full mt-1"
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" @selected($room->categories->contains($category))>
